let f =
  ...
  | NodeConnection(sinkNodeID) =>
    ...
    let sinkIndexInColumn =
      ListFindByIndexExn.f(column, node => node.id == sinkNodeID); // 2nd metric

    let node = Belt.List.getExn(column, sinkIndexInColumn);
    let nodeDisplay = NodeGetDisplayNibs.f(node.node, definitions, "en");

    let nibCollection =
      NodeIsFunctionDefinition.f(node.node)
        ? nodeDisplay.internalOutputs : nodeDisplay.inputs;
    let sinkIndex =
      ListFindByIndexExn.f(nibCollection, ({nib}) => nib == sink.nib); // 3rd metric
    (- sinkColumnIndex, sinkIndexInColumn, sinkIndex);
  };
};
